...
the invoice becomes a required item the provider supplies, and acceptance is not complete
without it. The runtime already retains all Active Service files for dispute access, so no
extra structure is needed.

Contract/SOW authority is authority-critical: re-confirm it live before activation.
