...
Capture escalation thresholds, information and authorization requests, payment exceptions, agent pause/resume rules, notification channels, and who on the buyer side owns each escalation path. For a single decision-maker, escalations route back to the same human; for multiple decision-makers, route them to the relevant invited roles (for example finance or approver).
...
Only this fixed set of buyer action types is eligible: `sign`, `accept` (delivery acceptance), `payment`, `dispute`. Operational actions (discovery answers, meeting, message) are not eligible and stay with the agent; the human can still take any single flow over ad-hoc with `$belong-operate-buying-flow`.
